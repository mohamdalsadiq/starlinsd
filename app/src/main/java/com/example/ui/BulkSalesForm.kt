...
@Composable internal fun BulkSalesForm(premium: Int, dismiss: () -> Unit, save: (String, List<Pair<Int, Long>>, String) -> Unit) {
    ...
    Form("دخل بعدد الأجهزة", dismiss, {
        save(id, parsed.filter { it.first!! > 0 }.map { it.first!! to it.second!! }, payment)
    }, valid) {
        ...
        counts.indices.forEach { i ->
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                Column(Modifier.weight(1f)) { Field("العدد ${i + 1}", counts[i], { value -> counts = counts.toMutableList().also { it[i] = value } }) }
        ...
        HorizontalDivider()
        MoneyLine("المبلغ الذي سيُضاف ${if (payment == "BANK") "بنكك" else "كاش"}", total, strong = true)
        if (payment == "BANK") MoneyLine("القيمة المكافئة للكاش", Money.bankToCash(total, premium))
